...
tf.app.flags.DEFINE_string(
    'development_dataset_path', '../../data/development.hdf5',
    'Directory where checkpoints and event logs are written to.')

tf.app.flags.DEFINE_string(
    'enrollment_dataset_path', '../../data/enroll_evaluation.hdf5',
    'Directory where checkpoints and event logs are written to.')
...
def main(_):
    ...
    with tf.Session(graph=graph, config=tf.ConfigProto(allow_soft_placement=True)) as sess:
        ...
        enroll_labels = fileh.root.label_enrollment[:]
        print(np.unique(enroll_labels).shape[0])
        NumClasses = np.unique(enroll_labels).shape[0]
        NumLogits = 128
        MODEL = np.zeros((NumClasses, NumLogits), dtype=np.float32)

        # Go through the speakers.
        for speaker_id, speaker_class in enumerate(enroll_labels):
            ...
            speaker_enrollment, label_enrollment = fileh.root.utterance_enrollment[speaker_id, :, :,
                                                     :],np.full(20,[fileh.root.label_enrollment[speaker_id]])
            #print(speaker_enrollment.shape)
            #print(label_enrollment[:])
            # Just adding a dimention for 3D convolutional operations.
            speaker_enrollment = speaker_enrollment[None, :, :, :,None]            
            #print (speaker_enrollment.shape)
            #print(label_enrollment.reshape([label_enrollment.shape[0], 1]))
            # Evaluation
            feature = sess.run(
